...
internal actual object LogPlatform {
    ...
    actual fun writeConsole(level: LogLevel, tag: String, text: String) {
        val normalizedTag = tag.ifBlank { Logger.DEFAULT_TAG }.take(23)
        when (level) {
            LogLevel.DEBUG -> Log.d(normalizedTag, text)
            LogLevel.INFO -> Log.i(normalizedTag, text)
            LogLevel.WARN -> Log.w(normalizedTag, text)
            LogLevel.ERROR -> Log.e(normalizedTag, text)
        }
    }
    ...
    private fun createOutputFile(
        ...
        return when (val platformFile = outputDir.androidFile) {
            is AndroidFile.FileWrapper -> {
                ensureDirectory(platformFile.file)
                val outputFile = File(platformFile.file, fileName)
            ...
            is AndroidFile.UriWrapper -> {
                val directory = DocumentFile.fromTreeUri(FileKit.context, platformFile.uri)
                    ?: DocumentFile.fromSingleUri(FileKit.context, platformFile.uri)
                    ?: throw IllegalStateException("导出目录不可访问")
                require(directory.isDirectory) { "导出目录不是文件夹" }

                directory.findFile(fileName)?.delete()
                val outputFile = directory.createFile("application/zip", fileName)
                    ?: throw IllegalStateException("无法创建导出文件: $fileName")
                PlatformFile(AndroidFile.UriWrapper(outputFile.uri))
            }
        }
    }

    private fun openOutputStream(file: PlatformFile): OutputStream {
        return when (val platformFile = file.androidFile) {
            is AndroidFile.FileWrapper -> FileOutputStream(platformFile.file)
            is AndroidFile.UriWrapper -> {
                FileKit.context.contentResolver.openOutputStream(platformFile.uri)
                    ?: throw IllegalStateException("无法打开导出输出流")
            }
        }
    }
