...
class WorkspaceRuntimeResolverTest {
    @Test
    fun openReusesStoredSnapshotWithoutRefreshing() {
        ...
        assertEquals(before?.toSummary(), context.snapshot)
    ...
    @Test
    fun openRebuildsMissingSnapshotAndPersistsNewSummary() {
        val workspaceDir = createTempDirectory("dexclub-core-open-missing-snapshot")
        val dexFile = workspaceDir.resolve("1.dex")
        dexFile.writeText("before")
        ...
        val prepared = env.bootstrapper.prepare(dexFile.toString())
        env.store.initialize(prepared.ref.workdir, prepared.workspace, prepared.target, prepared.snapshot)
        workspaceDir.resolve(".dexclub/targets").resolve(prepared.target.targetId).resolve("snapshot.json").deleteExisting()
        ...
        val persisted = env.store.loadSnapshot(workspaceDir.toString(), prepared.target.targetId)
        assertNotNull(persisted)
        assertEquals(context.snapshot, persisted.toSummary())
    }
    ...
    @Test
    fun openCanRecoverMissingSnapshotForLargeDexWithoutMaterializingWholeFile() {
