...
class DefaultWorkspaceServiceTest {
    ...
    @Test
    fun refreshRebuildsSnapshotExplicitly() {
        val workspaceDir = createTempDirectory("dexclub-core-service-refresh")
        val dexFile = workspaceDir.resolve("1.dex")
        dexFile.writeText("before")

        val service = createService()
        service.initialize(dexFile.toString())

        dexFile.writeText("after")

        val stale = service.open(WorkspaceRef(workspaceDir.toString()))
        val refreshed = service.refresh(stale)

        assertEquals(stale.activeTarget, refreshed.target)
        assertNotEquals(stale.snapshot.contentFingerprint, refreshed.snapshot.contentFingerprint)
        assertEquals(refreshed.snapshot, service.open(WorkspaceRef(workspaceDir.toString())).snapshot)
    }
