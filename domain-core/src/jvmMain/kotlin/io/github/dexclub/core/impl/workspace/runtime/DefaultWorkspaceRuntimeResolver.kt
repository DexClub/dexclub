...
internal class DefaultWorkspaceRuntimeResolver(
    ...
    override fun open(ref: WorkspaceRef): WorkspaceContext {
        ...
        inputResolver.resolveBoundInput(workdirPath, target.inputPath, target.inputType)
        val snapshot = loadSnapshotRecord(workdirPath, workdir, target)
    ...
    private fun loadSnapshotRecord(
        workdirPath: Path,
        workdir: String,
        target: TargetRecord,
    ): SnapshotRecord =
        runCatching { store.loadSnapshot(workdir, target.targetId) }.getOrNull()
            ?: refreshSnapshotRecord(workdirPath, target)
