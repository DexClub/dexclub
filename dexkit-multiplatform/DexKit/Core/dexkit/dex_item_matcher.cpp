    ...
    if (matcher->values()) {
        ...
        if (ptr == nullptr) {
            ...
            for (auto i = 0; i < matcher->values()->size(); ++i) {
                ...
                auto matcher_wrapper = matcher->values()->Get(i);
                auto type = matcher_wrapper->value_type();
                auto value = matcher_wrapper->value();
...
bool DexItem::IsUsingNumbersMatched(uint32_t method_idx, const schema::MethodMatcher *matcher) {
    ...
    if (ptr == nullptr) {
        ...
        */
        auto numbers = Numbers();
        auto using_numbers = matcher->using_numbers();
        for (int i = 0; i < using_numbers->size(); ++i) {
            auto NumberMatcherWrapper = using_numbers->Get(i);
            auto type = NumberMatcherWrapper->value_type();;
            EncodeNumber number{};
            switch (type) {
                case schema::Number::EncodeValueByte: {
                    number = EncodeNumber{
                            .type = BYTE,
                            .value = {.L8 = NumberMatcherWrapper->value_as<schema::EncodeValueByte>()->value()}
                    };
                    break;
                }
                case schema::Number::EncodeValueShort: {
                    number = EncodeNumber{
                            .type = SHORT,
                            .value = {.L16 = NumberMatcherWrapper->value_as<schema::EncodeValueShort>()->value()}
                    };
                    break;
                }
                case schema::Number::EncodeValueInt: {
                    number = EncodeNumber{
                            .type = INT,
                            .value = {.L32 = {.int_value = NumberMatcherWrapper->value_as<schema::EncodeValueInt>()->value()}}
                    };
                    break;
                }
                case schema::Number::EncodeValueLong: {
                    number = EncodeNumber{
                            .type = LONG,
                            .value = {.L64 = {.long_value = NumberMatcherWrapper->value_as<schema::EncodeValueLong>()->value()}}
                    };
                    break;
                }
                case schema::Number::EncodeValueFloat: {
                    number = EncodeNumber{
                            .type = FLOAT,
                            .value = {.L32 = {.float_value = NumberMatcherWrapper->value_as<schema::EncodeValueFloat>()->value()}}
                    };
                    break;
                }
                case schema::Number::EncodeValueDouble: {
                    number = EncodeNumber{
                            .type = DOUBLE,
                            .value = {.L64 = {.double_value = NumberMatcherWrapper->value_as<schema::EncodeValueDouble>()->value()}}
                    };
                    break;
                }
                default: abort();
