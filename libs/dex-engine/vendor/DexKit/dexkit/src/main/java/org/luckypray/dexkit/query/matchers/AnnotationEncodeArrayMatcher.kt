...
import org.luckypray.dexkit.InnerAnnotationEncodeValueMatcherWrapper
...
class AnnotationEncodeArrayMatcher : BaseQuery(), IAnnotationEncodeValue {
    ...
    override fun innerBuild(fbb: FlatBufferBuilder): Int {
        ...
        // wrapper
        val encodeValuesMatcherWrapper = encodeValuesMatcher?.map {
            InnerAnnotationEncodeValueMatcherWrapper.createAnnotationEncodeValueMatcherWrapper(
                fbb,
                it.type!!.value,
                (it.value as BaseQuery).build(fbb)
            )
        }
        val root = InnerAnnotationEncodeArrayMatcher.createAnnotationEncodeArrayMatcher(
            fbb,
            encodeValuesMatcherWrapper?.toIntArray()
                ?.let { InnerAnnotationEncodeArrayMatcher.createValuesVector(fbb, it) } ?: 0,
            matchType.value,
            rangeMatcher?.build(fbb) ?: 0
        )
