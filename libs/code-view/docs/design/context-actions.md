...
- 命中后是否允许保留现有选区
- 编辑态右键命中对象与菜单内容如何收口
- annotation 命中范围必须统一使用字符 offset，而不是直接复用 tree-sitter 的 UTF-8 byte offset；否则在中文等多字节文本前缀存在时，类名/方法名上下文菜单会漏命中
- annotation 命中对字符边界还要有轻微容错；否则点击在方法名末尾边界时，会出现能看到菜单但 `更多操作` 丢失的情况
...
- 超大反编译文件在 plain text token 降级下，类名/方法名 annotation 菜单是否仍然可用
...
- 状态：`进行中`
- 当前目标：修复 annotation 命中稳定性，并继续收口 annotation 上下文、selection 上下文和平台菜单边界
- 对应总进度：[`../status.md`](../status.md)
## 2026-08-02 补充

- 方法命中后的共享菜单现在把`复制方法签名`提升到一级菜单；类命中时同层支持`复制类签名`；如果同时存在文本选区，则额外显示`复制选中文本`。
- 方法签名复制内容统一使用完整 JVM / Dex member descriptor，例如 `Lcom/example/Demo;->test(I)V`；类签名复制内容统一使用 `Lcom/example/Demo;`，避免再退回仅复制展示名。
- Desktop annotation 右键命中不能依赖创建手势协程时捕获的滚动值；滚动后的命中必须读取实时 horizontal / vertical scroll，否则会出现“顶部能命中、下滑后相同方法丢失更多操作菜单”的问题。
- annotation offset 命中在字符边界保留轻量容错，优先查当前 offset，再退让到相邻字符，减少方法名尾部边界点击丢菜单。
