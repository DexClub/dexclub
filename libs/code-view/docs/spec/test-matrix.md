...
- 打开页内搜索框后输入查询词，确认 `Ctrl/Cmd + F`、命中计数、`Enter / Shift+Enter` 导航与 `Esc` 关闭符合预期
- 在页内搜索中连续切换上下项，确认命中计数、活动命中、范围选区、caret 与 reveal 保持落在同一项
- 清空页内搜索查询后，确认不会残留上一次命中的范围选区
