...
- Viewer 在大文件场景下必须基于虚拟纵向滚动偏移完成 reveal，不能再依赖真实超高布局节点
...
- 搜索框按 pane 独立工作
- Java 与 Smali 各自维护自己的查询词与当前命中
- mixed 模式下两个 pane 可以同时显示搜索框

### DexKit 回填

当前规范为：

- DexKit 字符串命中打开代码页后，应回填字符串查询并定位命中，但不自动显示顶部搜索框
- DexKit 类命中打开代码页后，应分别为 Java / Smali pane 回填各自查询并定位命中，但不自动显示顶部搜索框

### 键盘行为

当前第一版至少应支持：

- `Ctrl/Cmd + F` 打开当前 pane 搜索框并聚焦输入框
- `Enter` 跳到下一项
- `Shift + Enter` 跳到上一项
- `Esc` 关闭当前 pane 搜索框
...
- 搜索框应显示当前命中序号与总命中数
- `searchHighlight` 负责表达活动命中，保持当前更突出的搜索高亮色
- `inactiveSearchHighlights` 负责表达其他命中，使用更浅的搜索高亮色
- 活动命中切换时，命中计数、黄色活动命中、淡蓝范围选区、caret 与 reveal 应保持落在同一项
- 搜索命中的 caret 应落在命中尾部，而不是命中首部
- 查询被清空或当前无命中时，应清掉搜索驱动的范围选区，不能残留上一次命中
- 搜索框可见且存在命中时，应显示全部命中的分层高亮；当前活动命中更突出，其他命中更浅
