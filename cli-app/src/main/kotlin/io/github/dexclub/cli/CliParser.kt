...
internal class CliParser {
    ...
    private val commandParsers: Map<String, (List<String>) -> CliRequest> = buildMap {
        CliWorkspaceCommandCatalog.commands.forEach { metadata ->
            put(metadata.command) { tokens -> parseWorkspaceCommand(metadata, tokens) }
        }
        putAll(mapOf(
        "help" to ::parseHelp,
    ...
    private fun parseWorkspaceCommand(metadata: CliWorkspaceCommandMetadata, tokens: List<String>): CliRequest {
        if (tokens.size == 1 && CliHelp.isHelpFlag(tokens.single())) {
            return CliRequest.Help(metadata.command)
        }
        val parsed = CliParserSupport.parsePositionalAndFlags(
            tokens = tokens,
            allowJson = metadata.allowJson,
            usage = metadata.usage,
        )
        return when (metadata.parseMode) {
            CliWorkspaceCommandParseMode.InitInput -> {
            ...
            CliWorkspaceCommandParseMode.SwitchInput -> {
                if (parsed.positionals.size != 1) {
                    throw CliUsageError(
                        message = "missing required argument: <input>",
                        usage = metadata.usage,
            ...
            CliWorkspaceCommandParseMode.Workdir -> {
                if (parsed.positionals.size > 1) {
                    throw CliUsageError(
                        message = "too many positional arguments",
                        usage = metadata.usage,
                ...
                when (metadata.command) {
                    ...
                    "refresh" -> CliRequest.Refresh(parsed.positionals.singleOrNull(), parsed.outputFormat)
                    "inspect" -> CliRequest.Inspect(parsed.positionals.singleOrNull(), parsed.outputFormat)
                    else -> error("unsupported workspace command: ${metadata.command}")
                }
            }
        }
    }
...
internal object CliUsages {
    ...
    val init: String get() = CliWorkspaceCommandCatalog.require("init").usage
    val switch: String get() = CliWorkspaceCommandCatalog.require("switch").usage
    val targets: String get() = CliWorkspaceCommandCatalog.require("targets").usage
    val status: String get() = CliWorkspaceCommandCatalog.require("status").usage
    val gc: String get() = CliWorkspaceCommandCatalog.require("gc").usage
    val refresh: String get() = CliWorkspaceCommandCatalog.require("refresh").usage
    val inspect: String get() = CliWorkspaceCommandCatalog.require("inspect").usage
